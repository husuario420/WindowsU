	...
	PowerShell: (function() {
		let fn = function(command, callback, options) {
			...
			try {
				...
				function pushErrors() {
					try {
						// Debugging logs to inspect the command queue
						console.log("DEBUG: getCommandq(child):", getCommandq(child));
						console.log("DEBUG: getCommandq():", getCommandq());
						console.log("DEBUG: getCommandq(child)[0]:", getCommandq(child) ? getCommandq(child)[0] : "undefined");
						console.log("DEBUG: getCommandq()[0]:", getCommandq() ? getCommandq()[0] : "undefined");

						const commandQueueChild = getCommandq(child); // Queue for the child process
						const commandQueue = getCommandq(); // General command queue

						// First condition: errorBin is an empty string
						if (
							commandQueueChild &&
							commandQueueChild[0] &&
							commandQueueChild[0].errorBin === ''
						) {
							getPowerShellSession(child).err.push(commandQueueChild[0].errorBin);

						// Second condition: errorBin is not a new line, carriage return, or empty
						} else if (
							commandQueue &&
							commandQueue[0] &&
							commandQueue[0].errorBin &&
							commandQueue[0].errorBin.trim() !== '\n' &&
							commandQueue[0].errorBin.trim() !== '\r' &&
							commandQueue[0].errorBin.trim() !== '\r\n'
						) {
							const trimmedError = commandQueue[0].errorBin.toString().trim();
							getPowerShellSession(child).err.push(trimmedError);

							// Log the error if it exists and suppression is not enabled
							if (
								trimmedError !== '' &&
								commandQueue.length > 0 &&
								!(commandQueue[0].options && commandQueue[0].options.suppressErrors === true)
							) {
								Win.error(trimmedError);
							}
						}

						// Reset the errorBin to an empty string if the queue exists
						if (commandQueue && commandQueue[0]) {
							commandQueue[0].errorBin = '';
						}

						// Check if the queue for the child process has only one item
						if (commandQueueChild && commandQueueChild.length === 1) {
							checkIfDone({
								child: child,
								callback: callback,
								options: commandQueueChild[0] ? commandQueueChild[0].options : {},
								result: {
									out: getPowerShellSession(child).out,
									err: getPowerShellSession(child).err,
								},
							});
						}

						// Proceed to the next command in the queue
						setTimeout(() => {
							shiftQ();
							runNextInQ();
						}, 800);
						...
						console.error("Error in pushErrors function:", error);
					}
				}
